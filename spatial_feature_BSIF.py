...
base_path = r'D:\BoyangDeng\Biang\Graphics\BlueberryClassification\texturefilters'
bsif_filter_path = os.path.join(base_path, 'ICAtextureFilters_7x7_8bit.mat')
...
def bsif(img, mode='nh'):
    ...
    if mode == 'h' or mode == 'nh':
        bsifdescription = np.histogram(codeImg, bins=2**numScl, range=[0, 2**numScl + 1])[0]
    if mode == 'nh':
...
def main():
    ...
    if len(image.shape) == 3:
        image = image[:, :, 0]
    bsifdescription = bsif(image)
    print(bsifdescription)
    print(bsifdescription.shape)
